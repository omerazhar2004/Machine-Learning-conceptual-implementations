...
def partition_data(training_data: pd.DataFrame, f: int) -> list[pd.DataFrame]:
    ...
    if f > len(training_data):
        # This ensures that the number of partitions does not exceed the actual length of the training data
        f = len(training_data) 
    ...
    for i in range(f):
        ...
        if i < remainder:
            # This ensures that if the current partition number(when looping through number of partitions f) is less than the remainder then one extra row is added to it, this ensures that all partitions remain of a similar size with minor differences when it is impossible to ensure all have same number of rows.
            current_size += 1
...
def arrange_data_for_cv(partition_list: list[pd.DataFrame], f: int) \
    ...
    for i in range(len(partition_list)):
        ...
        # Ensures that all partitions before and after the chosen testing data in the partition_list are considered as training data
...
def cross_validate(nb: NaiveBayes, training_data: pd.DataFrame, f: int,
    ...
    # joining together the testing data from all folds which now has two new columns(Fold and PredictedClass) into one large data.
