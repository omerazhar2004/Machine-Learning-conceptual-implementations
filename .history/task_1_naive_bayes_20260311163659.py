...
class NaiveBayes:
    ...
    def train_model(self, training_data: pd.DataFrame):
        ...
        for class_value in self.class_info[1]:
            #Filter the entire training_data to choose only the rows with one type of class which is the class we are currently on when looping through class_info
            class_data = training_data[training_data[self.class_info[0]] == class_value]
            class_count = len(class_data)
            for feature_name in self.feature_info:
                #for counting the total number of each value for a specific feature_name and making a dictionary with that info, this feature_name would be the one we are currently on when looping through feature_info
    ...
    def predict(self, testing_data: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, row in df_copy.iterrows():
            ...
            #Converts the class_scores dictionary into pairs and sorts it which ensures the class with highest score comes first and is selected. Incase of a tie, class which comes first alphabetically gets selected.
    ...
    def retrieve_conditional_probability(self, class_value: str, feature_name: str, feature_value: str) -> float:
        key = (class_value, feature_name, feature_value)
        if key in self.conditional_probabilities:
            cond_prob = self.conditional_probabilities[key]
        else:
            cond_prob = 0
        return cond_prob    
