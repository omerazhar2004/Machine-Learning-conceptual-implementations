...
class NaiveBayes:
    ...
    def retrieve_class_probability(self, class_value: str) -> float:
        if self.prior_probabilities.has_key(class_value):
            class_prob = self.prior_probabilities.get(class_value)
        else:
            class_prob = 0
        return class prob float value
        return -1
    ...
    def retrieve_conditional_probability(self, class_value: str, feature_name: str, feature_value: str) -> float:
        if exists in self.conditional_probabilities:
            cond_prob = filter self.conditional_probabilities based on class_value, feature_name and feature_value passed in function
        else:
            cond_prob = 0 float
        return cond_prob    
