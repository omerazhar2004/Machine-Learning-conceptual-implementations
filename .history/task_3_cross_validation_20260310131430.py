...
def cross_validate(nb: NaiveBayes, training_data: pd.DataFrame, f: int,
    ...
    output_dataset = None
    partitioned_data = partition_func(training_data)
    prepared_data = prep_func(partition_data)
    fold_numbers = []
    for i in range(len(prepared_data)):
        sub_training_data = i[1]
        fold_number = i[0]
        fold_numbers.append(fold_number)
        sub_training_data_predicted = nb.predict(sub_training_data)
        sub_training_data_predicted["Fold"] = fold_numbers

        output_dataset.append(sub_training_data_predicted)
